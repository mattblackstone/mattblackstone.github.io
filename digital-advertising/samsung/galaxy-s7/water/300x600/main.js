  ...
  init: function(){
    ...
    function startAnimation() {
      if(adSeen){
        ...
        loadImage(splash_img, 'water-front-600x600-4x3.jpg', makeAnimatedSprite2);
    ...
    function frame0() {
      ...
      ai_sprite.every = 3; // fps = 60 / every
      ai_sprite.play();
      bu.removeClass(ai_canvas, 'alpha-0');

      setTimeout(frame2a, 1000);
    }

    function frame2a(){
      bu.addClass(splash_holder, 'trans-4000');
      bu.addClass(splash_holder, 'ease-out-expo');

      bu.addClass(device, 'trans-4000');
      bu.addClass(device, 'ease-out-cubic');
      bu.addClass(device_shadow, 'trans-4000');
      bu.addClass(device_shadow, 'ease-out-cubic');

      bu.addClass(device_holder, 'trans-500');
      bu.addClass(device_holder, 'ease-out-sine');
      bu.addClass(ai_canvas, 'trans-500');
      bu.addClass(ai_canvas, 'ease-in-sine');

      splash_sprite.every = 15;
      splash_sprite.setCrossfade(true);

      setTimeout(frame3, 750);
    }

    function frame3() {
      splash_sprite.play();

      bu.replaceClass(splash_holder, 'splash-start', 'splash-end');
      bu.replaceClass(device, 'device-start', 'device-end');
      bu.replaceClass(device_shadow, 'device-start', 'device-end');
      bu.addClass(ai_canvas, 'alpha-0');
      bu.removeClass(device_holder, 'alpha-0');

      setTimeout(frame4, 2750);
    }
    ...
    function makeAnimatedSprite2(){
      ...
      splash_sprite = new AnimatedSprite(splash_canvas, splash_img, 600, 600, 12);
    }

    function makeAnimatedSprite(){
      ai_sprite = new AnimatedSprite(ai_canvas, ai_img, 300, 180, 48);
      if ( isIE9 ) {
    ...
    loadImage(ai_img, 'water-side-300x180-8x6.jpg', makeAnimatedSprite);
    ...
    function AnimatedSprite(canvas, img, w, h, frames, record){
      ...
      self.setCrossfade = function(bool){
        if(bool){
          ...
          // canvas2.style.position = 'absolute'; // doesn't work
