...
//  bu.makeVarsFromIds(scope:object, holder:object, log:array, camelCase:boolean)
    ...
    function frame0(){
      var towerTop = parseInt(bu.getStyle(dom.tower).top, 10);
      var towerScale = 0.5;
      var downShift = Math.round(25 * towerScale);
      dom.blok1.classList.remove('blok-right');
      setTimeout(function(){
        dom.score.innerHTML = 98;
        bu.trace(bu.getStyle(dom.tower).top);
        dom.tower.style.top = (towerTop += downShift)+'px';
    ...
    function frame3(){
      ...
      setTimeout(function(){
        ...
        // dom.tagline.classList.remove('alpha-0');
      }, 1000);
      setTimeout(function(){
